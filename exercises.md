...
`RecursiveChunker` thử split văn bản theo thứ tự separator từ lớn đến nhỏ. Ví dụ:

```python
separators = ["\n## ", "\n### ", "\n\n", ". ", "? ", "! ", " ", ""]
```

Ý tưởng là ưu tiên tách theo cấu trúc có ý nghĩa trước, như heading markdown hoặc paragraph. Nếu một đoạn sau khi split vẫn quá dài, hàm `_split` tiếp tục gọi đệ quy với separator nhỏ hơn. Base case là khi text đã nhỏ hơn hoặc bằng `chunk_size`, hoặc khi không còn separator nào nữa thì fallback sang cắt cứng theo ký tự.
...
**Domain nhóm chọn:** FAQ hỗ trợ đặt món / giao đồ ăn.

Nhóm chọn domain này vì các câu hỏi của người dùng trong app giao đồ ăn thường có intent rõ ràng, ví dụ hủy đơn, đổi địa chỉ, hoàn tiền, mã giảm giá và theo dõi đơn hàng. Đây là domain phù hợp để thử embedding search vì nhiều câu hỏi có thể dùng từ khác nhau nhưng cùng ý nghĩa. Ngoài ra, domain này dễ thiết kế metadata như `category`, `intent`, `language`, `source`.
...
|---|--------------|-------|----------|-----------------|
| 1 | `FAQ_Huy_Don.md` | Nhóm tự tổng hợp từ use case app giao đồ ăn | 2,180 | `doc_id=faq_cancel`, `category=order`, `intent=cancel_order`, `language=vi` |
| 2 | `FAQ_Doi_Dia_Chi.md` | Nhóm tự viết dựa trên flow hỗ trợ khách hàng | 1,760 | `doc_id=faq_address`, `category=delivery`, `intent=change_address`, `language=vi` |
| 3 | `FAQ_Hoan_Tien.md` | Nhóm tự tổng hợp từ chính sách hoàn tiền phổ biến | 2,450 | `doc_id=faq_refund`, `category=payment`, `intent=refund`, `language=vi` |
| 4 | `FAQ_Ma_Giam_Gia.md` | Nhóm tự viết từ tình huống lỗi voucher | 1,920 | `doc_id=faq_voucher`, `category=promotion`, `intent=voucher_error`, `language=vi` |
| 5 | `FAQ_Trang_Thai_Don.md` | Nhóm tự viết dựa trên hành trình đơn hàng | 2,100 | `doc_id=faq_tracking`, `category=delivery`, `intent=track_order`, `language=vi` |
...
|----------|----------|-------------|------------|-------------------|
| `FAQ_Hoan_Tien.md` | FixedSizeChunker | 6 | 408 | Trung bình — đôi khi cắt ngang điều kiện hoàn tiền |
| `FAQ_Hoan_Tien.md` | SentenceChunker | 5 | 490 | Tốt — giữ câu đầy đủ |
| `FAQ_Hoan_Tien.md` | RecursiveChunker | 5 | 482 | Rất tốt — giữ heading và đoạn |
| `FAQ_Doi_Dia_Chi.md` | FixedSizeChunker | 4 | 440 | Trung bình — có đoạn bị cắt giữa flow |
| `FAQ_Doi_Dia_Chi.md` | SentenceChunker | 4 | 435 | Tốt — giữ các bước hướng dẫn |
| `FAQ_Doi_Dia_Chi.md` | RecursiveChunker | 4 | 445 | Rất tốt — giữ block hướng dẫn |
| `FAQ_Ma_Giam_Gia.md` | FixedSizeChunker | 5 | 384 | Trung bình — dễ tách lỗi và cách xử lý |
| `FAQ_Ma_Giam_Gia.md` | SentenceChunker | 4 | 480 | Tốt |
| `FAQ_Ma_Giam_Gia.md` | RecursiveChunker | 4 | 475 | Rất tốt |
...
|----------|----------|-------------|------------|-------------------|
| `FAQ_Hoan_Tien.md` | FixedSizeChunker | 6 | 408 | 6.5/10 — có keyword nhưng mất context |
| `FAQ_Hoan_Tien.md` | SentenceChunker | 5 | 490 | 8/10 — giữ câu tốt |
| `FAQ_Hoan_Tien.md` | RecursiveChunker | 5 | 482 | 9/10 — giữ heading + điều kiện |
| `FAQ_Doi_Dia_Chi.md` | FixedSizeChunker | 4 | 440 | 7/10 |
| `FAQ_Doi_Dia_Chi.md` | SentenceChunker | 4 | 435 | 8/10 |
| `FAQ_Doi_Dia_Chi.md` | RecursiveChunker | 4 | 445 | 9/10 |
| `FAQ_Ma_Giam_Gia.md` | FixedSizeChunker | 5 | 384 | 6.5/10 |
| `FAQ_Ma_Giam_Gia.md` | SentenceChunker | 4 | 480 | 8/10 |
| `FAQ_Ma_Giam_Gia.md` | RecursiveChunker | 4 | 475 | 9/10 |
...
|---|-------|-------------------------------|--------------------------|
| 1 | Tôi có thể hủy đơn sau khi nhà hàng đã nhận không? | Có thể hủy trong một số trường hợp, nhưng nếu nhà hàng đã bắt đầu chuẩn bị món thì có thể không hủy được hoặc vẫn bị tính phí. | `FAQ_Huy_Don.md`, chunk “Điều kiện hủy đơn” |
| 2 | Tôi nhập sai địa chỉ giao hàng thì phải làm sao? | Người dùng nên cập nhật địa chỉ trong app càng sớm càng tốt hoặc liên hệ hỗ trợ/tài xế nếu đơn đã được nhận. | `FAQ_Doi_Dia_Chi.md`, chunk “Đổi địa chỉ sau khi đặt” |
| 3 | Khi nào tôi được hoàn tiền? | Được hoàn tiền khi đơn bị hủy hợp lệ, thanh toán lỗi, món thiếu/sai hoặc nhà hàng không thể hoàn thành đơn. | `FAQ_Hoan_Tien.md`, chunk “Trường hợp được hoàn tiền” |
| 4 | Vì sao mã giảm giá không dùng được? | Có thể do voucher hết hạn, không đủ điều kiện đơn tối thiểu, sai phương thức thanh toán hoặc không áp dụng cho nhà hàng đó. | `FAQ_Ma_Giam_Gia.md`, chunk “Lỗi voucher thường gặp” |
| 5 | Với category payment, trường hợp nào được hoàn tiền? | Khi filter `category=payment`, câu trả lời nên lấy từ tài liệu hoàn tiền: thanh toán lỗi, đơn hủy hợp lệ, món thiếu/sai hoặc nhà hàng không hoàn thành đơn. | `FAQ_Hoan_Tien.md`, chunk “Trường hợp được hoàn tiền” |
